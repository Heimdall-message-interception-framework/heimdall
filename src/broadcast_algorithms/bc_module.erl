...
-export([generate_instruction/2, start/1, start_link/1, get_instructions/0, get_observers/0, get_properties/0, generate_instruction/1, bootstrap_wo_scheduler/0, bootstrap_wo_scheduler/1, needs_bootstrap_w_scheduler/0, bootstrap_w_scheduler/1, bootstrap_w_scheduler/2, stop_sut/0, stop_sut/1, abstract_state_mod/0]).
...
handle_call(bootstrap_wo_scheduler, _From, State) ->
    ...
    StartBCProcess = fun(Id) ->
        ...
        BCName = case BCType of
            causal_broadcast -> "cb";
            _ -> atom_to_list(BCType) end,
        ets:insert(pid_name_table, {Pid, Name++ "_" ++ BCName}),
        Pid end,
