...
-callback produce_sched_instruction(any(), any(), list(), list(), term()) -> {#instruction{} | undefined, term()}.
-callback produce_timeout_instruction(any(), list(), term()) -> {#instruction{} | undefined, term()}.
%% TODO: to add
...
handle_call({choose_instruction, MIL, SUTModule, SchedInstructions, History}, _From,
  ...
  {Instruction, SchedState2} = get_next_instruction(MIL, SUTModule, SchedInstructions, CommInTransit, Timeouts, Nodes, Crashed, SchedName, SchedState1),
...
-spec get_next_instruction(pid(), atom(), [#abstract_instruction{}], _, _, _, _, _, _) -> {#instruction{}, term()}.
get_next_instruction(MIL, SUTModule, SchedInstructions, CommInTransit, Timeouts, Nodes, Crashed, SchedName, SchedState) ->
%%  Crashed are the transient ones
  ...
  {NextInstruction, SchedState1} = case KindInstruction of
                      ...
                      sched_instruction ->
                        SchedName:produce_sched_instruction(MIL, SchedInstructions, CommInTransit, Timeouts, SchedState);
                      timeout_instruction ->
                        SchedName:produce_timeout_instruction(MIL, Timeouts, SchedState);
                      node_connection_instruction ->
                        SchedName:produce_node_connection_instruction(MIL, Nodes, Crashed, SchedState)
                    end,
%%  in case an impossible kind was chose, we simply retry -> TODO: do better
  case NextInstruction of
    undefined -> get_next_instruction(MIL, SUTModule, SchedInstructions, CommInTransit, Timeouts, Nodes, Crashed, SchedName, SchedState1);
